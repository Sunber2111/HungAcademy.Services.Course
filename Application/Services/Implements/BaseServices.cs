...
{
    public class BaseServices<TModel, TEntity> : IBaseServices<TModel, TEntity>
        where TModel : class
        where TEntity : class
    {
        ...
        public async Task<List<TModel>> GetAllAsync()
        {
            try
            {
                using (var unitOfWork = NewDbContext())
                {
                    ...
                    return _mapper.Map<IEnumerable<TEntity>, List<TModel>>(listE);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<TModel> InsertAsync(TModel item)
        {
            try
            {
                using (var unitOfWork = NewDbContext())
                {
                    ...
                    // Save change to datacontext.
                    var success = await unitOfWork.SaveChangeAsync();

                    if(success)
                        return _mapper.Map<TModel>(entityResult);
                    
                    throw new Exception("Problem when saving");
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<TModel> UpdateAsync(TModel item, object id)
        {
            try
            {
                using (var unitOfWork = NewDbContext())
        ...
        public async Task<List<TModel>> GetAsync(Expression<Func<TEntity, bool>> filter)
        {
            try
            {
                using (var unitOfWork = NewDbContext())
                {
                    var repository = unitOfWork.Repository<TEntity>();
                    var listE = await repository.GetManyAsync(filter);
        ...
        public async Task<TEntity> GetByIdEntityAsync(object id)
        {
           try
            {
                using (var unitOfWork = NewDbContext())
        ...
        public async Task<TModel> InsertAsync(TEntity item)
        {
            try
            {
                using (var unitOfWork = NewDbContext())
                {
                    ...
                    // Save change to datacontext.
                    var sucess = await unitOfWork.SaveChangeAsync();
                    if(sucess)
                        return _mapper.Map<TModel>(entityResult);
                    throw new Exception("Problem when Saving");
                }
        ...
        public async Task<TEntity> InsertEntityAsync(TEntity item)
        {
            try
            {
                using (var unitOfWork = NewDbContext())
                {
                    ...
                    var entity = repository.Insert(item);
